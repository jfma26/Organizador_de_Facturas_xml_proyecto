 ...
 * @author robert
 */
public class GNegocioProveedor extends javax.swing.JFrame {
    private DefaultTableModel model;
    private String Comprador="";
    private String VDatos[];
    private int conta=0;
    public GNegocioProveedor(String usuario) {
        try {
            VDatos=new String[15];
            Comprador=usuario;
            initComponents();
            setLocationRelativeTo(null);
            setTitle("Gasto Negocio por Fecha y Proveedor");
            Date dato = new Date();
            jDateChooser1.setMaxSelectableDate(dato);
            jDateChooser2.setMaxSelectableDate(dato);
            llenarCombo();
        } catch (SQLException ex) {
            Logger.getLogger(GNegocioProveedor.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void formatot() {
        String datos[][] = {};
        String Col[] = {"Num Factura", "Proveedor","Fecha","Mercaderia","Arriendo"
                ,"Servicios Basicos","Sueldos","Movilizacion Viaticos","Capacitacion"
                ,"Suministro de Oficina","Herramienta de Trabajo","Otros", "IVA","Total sin IVA","Total con IVA"};
    ...
    public void ExtrearFactura(String fecha1, String fecha2,String Proveedor) throws SQLException {
        ...
        ConexionBase cont = new ConexionBase();
        for(int i=1;i<=9;i++){
        String sentencia = "select Detalle_Factura_Negocio.Valor_Con_IVA from Detalle_Factura_Negocio,Factura_Negocio where Detalle_Factura_Negocio.Cod_Tipo_Negocio='"+i+"'"
                + " and Detalle_Factura_Negocio.Cod_Factura_Negocio=Factura_Negocio.Cod_Factura_Negocio and "
                +" Detalle_Factura_Negocio.Cod_Factura_Negocio='"+VDatos[0]
                + "' and Detalle_Factura_Negocio.RUC_Proveedor=Factura_Negocio.RUC_Proveedor and "
                + "Factura_Negocio.Fecha_Factura  BETWEEN '"+ fecha1 +"'  AND '"
                + fecha2 +"' and Detalle_Factura_Negocio.RUC_Proveedor='"+Proveedor+"' and Factura_Negocio.RUCoCI_Comprador='"+Comprador+"';";
        ResultSet resultado = cont.SelectDB1(sentencia);
        while (resultado.next()){
            conIva=(resultado.getString("Detalle_Factura_Negocio.Valor_Con_IVA"));
        }
    ...
    private void obtenerTabla(String fecha1, String fecha2,String Proveedor){
        ...
        try {
            ConexionBase cont = new ConexionBase();
            String sentencia = "select Cod_Factura_Negocio,RUC_Proveedor,Fecha_Factura,Valor_Sin_IVA,IVA,Valor_Con_IVA from Factura_Negocio where "
                    + "Fecha_Factura  BETWEEN '"+ fecha1 +"'  AND '"
                    + fecha2 +"' and RUC_Proveedor='"+Proveedor+"' and  RUCoCI_Comprador='"+Comprador+"';";
            ResultSet resultado = cont.SelectDB1(sentencia);
            while (resultado.next()){
                VDatos[0]=resultado.getString("Cod_Factura_Negocio");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(GNegocioProveedor.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
